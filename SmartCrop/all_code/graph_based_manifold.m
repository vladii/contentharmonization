...
function mapstage2 = graph_based_manifold (file_name, outpath)
    addpath('./others/');
    %%------------------------set parameters---------------------%%
    ...
    resize_factor = 256;
    imname = file_name;
    file_name = substr(imname, rindex(imname, '/')+1);

    [input_im,w]=removeframe(imname, resize_factor);% run a pre-processing to remove the image frame

    %%------- resize input image ---------%%

    outname_scaled=[outpath file_name(1:end-4) '_scaled' '.png'];
    imwrite(input_im, outname_scaled);
...
% assign the saliency value to each pixel     
     ...
     outname=[outpath file_name(1:end-4) '_stage1' '.png'];
...
% assign the saliency value to each pixel
    ...
    outname=[outpath file_name(1:end-4) '_stage2' '.png'];
