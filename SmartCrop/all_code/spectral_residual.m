...
function spectral_residual(impath, viewport_height, viewport_width, outpath)
    ...
    submatrix_points(1:(ORIGINAL_HEIGHT-viewport_height), 1:(ORIGINAL_WIDTH - viewport_width)) = ...
        nrPointsSubmatrix((1:(ORIGINAL_HEIGHT-viewport_height)) + viewport_height, (1:(ORIGINAL_WIDTH - viewport_width)) + viewport_width);

    submatrix_points(2:(ORIGINAL_HEIGHT-viewport_height), 1:(ORIGINAL_WIDTH - viewport_width)) -= ...
        nrPointsSubmatrix((2:(ORIGINAL_HEIGHT-viewport_height)) - 1, (1:(ORIGINAL_WIDTH - viewport_width)) + viewport_width);

    submatrix_points(1:(ORIGINAL_HEIGHT-viewport_height), 2:(ORIGINAL_WIDTH - viewport_width)) -= ...
        nrPointsSubmatrix((1:(ORIGINAL_HEIGHT-viewport_height)) + viewport_height, (2:(ORIGINAL_WIDTH - viewport_width)) - 1);

    submatrix_points(2:(ORIGINAL_HEIGHT - viewport_height), 2:(ORIGINAL_WIDTH - viewport_width)) += ...
        nrPointsSubmatrix((2:(ORIGINAL_HEIGHT-viewport_height)) - 1, (2:(ORIGINAL_WIDTH-viewport_width)) - 1);

    [r_max_arr, c_max_arr] = find(submatrix_points == max(submatrix_points(:)));
    r_max = r_max_arr(1);
    c_max = c_max_arr(1);

    %% Crop image.
    originalImage = imread(impath);
    Croppedimage = originalImage(r_max:r_max+viewport_height-1, c_max:c_max+viewport_width-1, :);

    outname = [outpath imname(1:end-4) '_viewport' '.png'];
    imwrite(Croppedimage, outname);
end
